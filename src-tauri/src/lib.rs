...
async fn launch_game_inner(
    ...
    let (mc_base_version, loader, loader_version) = {
        ...
        if ldr_ver
            .as_deref()
            .is_some_and(crate::minecraft::loader::is_stale_fabric_default)
        {
            ldr_ver = Some(crate::minecraft::loader::FABRIC_FALLBACK.to_string());
        }
    ...
    let java_path = if let Some(path) = &launcher_state.java_path {
        if path.trim().is_empty() {
            ...
            let mut is_ok = false;
            if let Ok(out) = crate::process::hidden_command(path).arg("-version").output() {
...
async fn create_instance(
    ...
    let pinned_loader_version = match loader_version.as_deref() {
        Some(version)
            if !version.is_empty()
                && version != "latest"
                && !crate::minecraft::loader::is_stale_fabric_default(version) =>
        {
            Some(version.to_string())
        }
        _ => None,
    };
    let selected_loader_version = match pinned_loader_version {
        Some(version) => version,
        None if selected_loader == "fabric" => {
            crate::minecraft::loader::latest_fabric_loader(&base_version).await
        }
...
async fn download_instance_file(
    ...
    // 1. Check central deduplication CAS cache in kryo_objects by URL hash
    use sha2::{Digest, Sha256};
    let mut hasher = Sha256::new();
    hasher.update(url.as_bytes());
    let url_hash = format!("{:x}", hasher.finalize());

    let cache_dir = mc_dir.join("kryo_objects").join(&url_hash[0..2]);
    let cache_file = cache_dir.join(&url_hash);

    if cache_file.exists() {
        if crate::minecraft::dedup::link_or_copy(&cache_file, &dest_path).is_ok() {
            println!(
                "[DEDUP] Reused cached instance file from kryo_objects for: {}",
                subpath
            );
            return Ok(subpath);
        }
    ...
    let resp = client
        ...
        .header(
            "User-Agent",
            concat!("KryoClient/", env!("CARGO_PKG_VERSION")),
        )
        .send()
        .await
...
mod tests {
    ...
    fn test_is_safe_jvm_arg() {
        ...
        assert!(is_safe_jvm_arg("-Dminecraft.launcher.brand=kryoclient"));
    ...
    fn test_addons_path_sanitization() {
        let base = Path::new("/tmp/kryo_test_base");
