...
/// Returns the central CAS (Content-Addressed Storage) directory in .minecraft/kryo_objects
pub fn get_objects_dir() -> PathBuf {
    let mc_dir = crate::minecraft::versions::get_minecraft_dir();
    mc_dir.join("kryo_objects")
}
...
mod tests {
    ...
    fn test_link_or_copy() {
        let tmp = std::env::temp_dir().join("kryo_test_dedup");
        ...
        f.write_all(b"hello kryoclient dedup").unwrap();
        drop(f);

        link_or_copy(&src, &dst).unwrap();
        assert!(dst.exists());
        assert_eq!(fs::read_to_string(&dst).unwrap(), "hello kryoclient dedup");
