...
pub fn get_legacy_source_candidates(app_handle: Option<&tauri::AppHandle>) -> Vec<PathBuf> {
    ...
    if let Some(data) = dirs::data_dir() {
        candidates.push(data.join("com.kryoclient.launcher"));
        candidates.push(data.join("kryoclient"));
        candidates.push(data.join(".kryoclient"));
    }

    if let Some(config) = dirs::config_dir() {
        candidates.push(config.join("com.kryoclient.launcher"));
        candidates.push(config.join("kryoclient"));
        candidates.push(config.join(".kryoclient"));
    }

    if let Some(home) = dirs::home_dir() {
        candidates.push(home.join(".kryoclient"));
        candidates.push(home.join(".config").join("kryoclient"));
        candidates.push(home.join(".config").join("com.kryoclient.launcher"));
        candidates.push(home.join(".local").join("share").join(".kryoclient"));
    }
...
mod tests {
    ...
    fn test_migration_logic() {
        let tmp = std::env::temp_dir().join("kryo_test_migration_full");
