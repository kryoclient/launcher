...
import type { KryoAddon } from "./types";

export async function loadAddonFromDisk(
  addonId: string,
): Promise<KryoAddon | null> {
  try {
    ...
    try {
      ...
      const addonInstance: KryoAddon = module.default || module;
    ...
    throw new Error(`Addon '${addonId}' does not export a valid KryoAddon`);
  } catch (error: any) {
...
export function injectAddonStyle(addonId: string, cssContent: string) {
  const styleId = `kryo-addon-style-${addonId}`;
