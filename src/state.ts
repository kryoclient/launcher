...
async function safeInvoke<T>(
  ...
  try {
    ...
    console.error(`[KryoClient State] Error in "${cmd}":`, error);
    return fallback;
  }
}
...
export const useLauncherStore = create<LauncherStore>((set, get) => ({
  ...
  addSkinToWardrobe: async (
    ...
    try {
      ...
      console.error("[KryoClient State] Failed to add skin to wardrobe:", error);
      throw error;
    }
  },
  ...
  applySkin: async (profileId: string, skinId: string) => {
    try {
      ...
      console.error("[KryoClient State] Failed to apply skin:", error);
      throw error;
    }
  },
  ...
  setActiveCape: async (profileId: string, capeId: string | null) => {
    try {
      ...
      console.error("[KryoClient State] Failed to set active cape:", error);
      throw error;
    }
  },
}));
