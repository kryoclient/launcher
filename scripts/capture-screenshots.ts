...
const PORT = 9237;
const USER_DATA_DIR = `/tmp/chrome-kryo-sc-${Date.now()}`;
...
async function preparePage(client: CDPClient) {
  ...
  await client.evaluate(`
    const launcherStore = window.KryoClient.useLauncherStore;
    const addonStore = window.KryoClient.useAddonStore;
...
async function main() {
  ...
  try {
    ...
    await client.captureScreenshot("kryoclient-dashboard.png");
    ...
    `);
    await sleep(800);
    await client.captureScreenshot("kryoclient-settings.png");
    ...
    `);
    await sleep(800);
    await client.captureScreenshot("kryoclient-addons.png");
    ...
    await client.evaluate(`
      ...
      if (consoleContainer) {
        consoleContainer.innerHTML = \`
          <div class="mb-2"><span class="text-gray-500">[13:37:00]</span> <span class="text-blue-400">[KryoClient:Core] Launcher initialized in 142 ms (RAM: 38 MB)</span></div>
          <div class="mb-2"><span class="text-gray-500">[13:37:01]</span> <span class="text-blue-400">[KryoClient:Addons] Active addons: 3D Skin Viewer v2.0.0, Discord RPC v1.1.0</span></div>
          <div class="mb-2"><span class="text-gray-500">[13:37:02]</span> <span class="text-gray-200">[Minecraft] Setting user: KryoPlayer (UUID: c0ffee00-1337-4242-beef-000000000001)</span></div>
          ...
          <div class="mb-2"><span class="text-gray-500">[13:37:06]</span> <span class="text-green-400">[Minecraft] Game started successfully! Connected to KryoClient</span></div>
        \`;
      }
    `);
    await sleep(500);
    await client.captureScreenshot("kryoclient-console.png");
